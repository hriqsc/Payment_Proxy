...
async fn run_health_checker(
    ...
    loop{
        let mut con = redis::Client::open(redis_address.clone())?.get_connection()?;

        let df_resp = hc_client.get(df_address.clone()).send().await?;
        let fb_resp = hc_client.get(fb_address.clone()).send().await?;
